...
void pc_event(const int pc)
{
    ...
    if (stream_check)
    {
        std::string fname("../hexfiles/"+ std::string(benchmark) +".pc.txt");
...
void wb_event(const int addr, const int data)
{
    ...
    if (stream_check)
    {
        std::string fname("../hexfiles/"+ std::string(benchmark) +".wb.txt");
        static std::ifstream f(fname);
        if (!f.is_open())
        {
            std::cerr << "Failed to open file: " << fname << std::endl;
...
void ls_event(const int op, const int addr, const int data)
{
    ...
    if (stream_check)
    {
        std::string fname("../hexfiles/"+ std::string(benchmark) +".ls.txt");
        static std::ifstream f(fname);
        if (!f.is_open())
        {
            std::cerr << "Failed to open file: " << fname << std::endl;
...
int main(int argc, char **argv)
{
    ...
    std::string const hex_file_name ("../hexfiles/" + std::string(benchmark) + ".hex");
    ...
    while (!top->done && !(interrupt && main_time >= stop_time))
    {
        ...
        //if (main_time % 1000000 == 0) std::cout << "Time is now: " << main_time << std::endl;
        //if (main_time > 229690375) break; 
        if (dump)
            tfp->dump(main_time);
    ...
    std::cout << std::dec
              ...
              << "\nCPI: " << (float)cycle_count / instruction_count 
              << "\nIPC: " << (float)instruction_count / cycle_count << std::endl;
